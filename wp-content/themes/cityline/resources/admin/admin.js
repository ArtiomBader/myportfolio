...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".cityline-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.cityline_admin) {
    return;
  }

  var _window$cityline_admi = window.cityline_admin,
      builderStatusData = _window$cityline_admi.builderStatusData,
      getStartedData = _window$cityline_admi.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.cityline-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".cityline-admin-big-notice").addClass("processing");
      $(".cityline-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".cityline-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.cityline-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".cityline-admin-big-notice");
  $custom_close_button = $notice_root.find(".cityline-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".cityline-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.cityline_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
